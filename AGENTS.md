...
npm run test            # Unit + integration tests (excludes long-running BlockHeadersClient.test.ts)
npm run test:connections # Long-running BlockHeadersClient.test.ts test (needs live network)
...
| `tests/BlockHeadersDatabase.test.ts` | Unit and integration tests for the header store and sorting |
