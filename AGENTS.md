...
- **Abort signal strategy**: Anything awaited that could block shutdown (sleeps, fetches, pings, node operations) must be cancellable via an `AbortSignal` rather than a flag, so `BlockHeadersClient.stop()` can shut down almost instantly. The stop path aborts the client signal, which propagates through `combineAbortControllers` (in `src/utils/util.ts`) into per-operation combined signals.
